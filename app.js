...
// Фикс для десктопного Telegram WebApp: textarea не получает фокус при клике.
// Принудительно вызываем focus() при mousedown чтобы обойти баг.
document.addEventListener('mousedown', e => {
    const el = e.target.closest('textarea, input[type="text"], input[type="email"], input[type="number"]');
    if (el) {
        setTimeout(() => {
            el.focus();
            // Ставим курсор в конец если поле уже содержит текст
